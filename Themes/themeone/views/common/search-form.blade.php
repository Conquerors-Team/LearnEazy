...
<div class="row">
        ...
        {{ Form::label('institute_id', getphrase('institute')) }}
        ...
        @if(checkRole(getUserGrade(1)) && empty($show_batch_assigned))
        <fieldset class="form-group col-lg-3">
        {{ Form::label('batch_assigned', getphrase('assigned')) }}?
        <span class="text-red">*</span>
        <?php
        $batch_assigned_opts = [
            '' => 'All',
        ...
        ?>
        {{Form::select('batch_assigned', $batch_assigned_opts, null, ['class'=>'form-control', 'id'=>'batch_assigned'
        ])}}
        </fieldset>
        @endif

        @if( empty( $show_content_types ) )
        ...
        ?>
        {{Form::select('content_type', $content_types, null, ['class'=>'form-control', 'id'=>'content_type',
        ...
        @if( empty( $show_subjects ) )
        <fieldset class="form-group col-lg-3">
        {{ Form::label('subject_id', getphrase('subject')) }}
        <span class="text-red">*</span>
        {{Form::select('subject_id', $subjects, null, ['class'=>'form-control','onChange'=>'getSubjectChapters()', 'id'=>'subject',
        	'ng-model'=>'subject_id',
        	'required'=> 'true',
        ...
        @if( empty( $show_topics ) )
        <fieldset class="form-group col-lg-3">
        {{ Form::label('topics', 'Topic') }}
        {{Form::select('topic_id', $topics, null, ['class'=>'form-control', 'id'=>'topic_id', 'onChange' => 'getChaptersTopicsSubtopics()' ])}}
        </fieldset>
        @endif

        @if( empty( $show_sub_topics ) )
        <fieldset class="form-group col-lg-3">
        {{ Form::label('sub_topic', 'Subtopic') }}
        {{Form::select('sub_topic_id', $sub_topics, null, ['class'=>'form-control', 'id'=>'sub_topic_id' ])}}
        </fieldset>
        @endif

        @if( empty( $show_question_actegory ) )
        <fieldset class="form-group col-lg-3">
        {{ Form::label('question_category_id', 'Question category') }}
        {{Form::select('question_category_id', $categories, null, ['class'=>'form-control', 'id'=>'question_category_id' ])}}
        </fieldset>
        @endif

        @if( empty( $show_difficulty_level ) )
        <fieldset class="form-group col-lg-3">
        {{ Form::label('difficulty_level', 'Difficulty level') }}
        {{Form::select('difficulty_level', $difficulty_levels, null, ['class'=>'form-control', 'id'=>'difficulty_level' ])}}
        </fieldset>
        @endif
