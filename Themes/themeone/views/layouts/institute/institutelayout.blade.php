...
<body ng-app="academia" @if(env('DISABLE_RIGHTCLICK')) oncontextmenu="return false;" @endif>
 ...
 ?>
	<div id="wrapper" class="{{$class}}">
		...
		<nav class="navbar navbar-custom navbar-fixed-top" role="navigation">
			...
			<ul class="nav navbar-right top-nav">
				<?php
				 	 ...
				 	 ?>
				 	 <li style="color: white;"><font size="4px;">{{$institute->institute_name}}</font>, {{ $institute->institute_address }}</li>
				<li class="dropdown">
		...
		?>
		@if($validity_days_left <= EXPIRE_NOTIFICATION_BEFORE)
		<div class="alert alert-info demo-alert">
		&nbsp;&nbsp;&nbsp;<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  			@if( empty( $user->valid_until) )
  				<b style="color: red; font-size: 16px;">Free trial days left <b>{{$validity_days_left}}</b>. Click <a href="{{route('packages.renew')}}" title="Renew">here</a> to renew it.</b>
  			@elseif( $validity_days_left <= 0)
  				<b style="color: red; font-size: 16px;">Your validity expired. Please contact administrator. Click <a href="{{route('packages.renew')}}" title="Renew">here</a> to renew it.</b>
  			@else
  				<b style="color: red; font-size: 16px;">Your validity expires soon. Please contact administrator. Click <a href="{{route('packages.renew')}}" title="Renew">here</a> to renew it.</b>
  			@endif
		</div>
		@endif
		...
		<aside class="left-sidebar">			<div class="collapse navbar-collapse navbar-ex1-collapse">				<ul class="nav navbar-nav side-nav">					<li {{ isActive($active_class, 'dashboard') }}>
					...
					<ul id="exams" class="collapse sidemenu-dropdown">
							...
							@if(checkRole(getUserGrade(2), 'share_exam_series'))
					...
					<ul id="lms" class="collapse sidemenu-dropdown">
							...
							@if(checkRole(getUserGrade(2), 'share_lms_contents'))
							...
							@endif
							@if(checkRole(getUserGrade(8), 'share_lms_notes'))
					...
					@if(canDo('sms_alerts') || canDo('email_alerts'))
					<li {{ isActive($active_class, 'notifications') }} >
						<a href="{{route('batch.email-sms-alerts')}}" ><i class="fa fa-fw fa-bell" aria-hidden="true"></i>
					{{ getPhrase('alerts') }} </a>
					</li>
					@endif
