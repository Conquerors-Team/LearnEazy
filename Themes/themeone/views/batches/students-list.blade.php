...
<div id="page-wrapper" ng-controller="batchesController">
			...
			<div class="container-fluid">
				...
				@include('common.search-form', ['url' => url()->current(), 'show_content_types' => 'FALSE', 'show_subjects' => 'FALSE', 'show_chapters' => 'FALSE', 'show_topics' => 'FALSE', 'show_sub_topics' => 'FALSE', 'show_question_actegory' => 'FALSE', 'show_difficulty_level' => 'FALSE'])
				@endif
				...
				$assigned = '';
				if ( ! empty( request('batch_assigned') )) {
					$assigned = request('batch_assigned');
				}
				?>
				<div class="panel panel-custom" ng-init="setPreSelectedData('{{$record->user_id}}','{{$institute_id}}','{{$record->id}}', '{{$assigned}}')">
   ...
   <div>
    ...
    <thead>
        ...
        <th style="border:1px solid #000;text-align: center;"><b>{{getPhrase('class')}}</b></th>
    ...
    <tr ng-repeat="user in result_data | filter:search track by $index">
            ...
            <td style="border:1px solid #000;text-align: center;">@{{user.classname}}</td>
