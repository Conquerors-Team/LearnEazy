...
class QuizResult extends Model
{
  use HasSlug;
    ...
    {
        parent::boot();
        QuizResult::observe(new \App\Observers\UserActionsObserver);
    }
 ...
 // dd($result);
         return $result;
            ...
            $institute_id  = adminInstituteId();
             ...
             $query = 'select count(qr.quiz_id) as total, q.title as quiz_title from quizzes q, quizresults qr where qr.quiz_id = q.id and qr.institute_id = '.$institute_id.' group by qr.quiz_id';


             if($type=='paid')
             {
                  $query = 'select count(qr.quiz_id) as total, q.title as quiz_title from quizzes q, quizresults qr where qr.quiz_id = q.id and q.is_paid=1 and qr.institute_id = '.$institute_id.' group by qr.quiz_id';
             }
