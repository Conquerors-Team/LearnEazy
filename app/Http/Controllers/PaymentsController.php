  ...
  public $payment_records = [];
    ...
    public function paynow(Request $request, $slug)
    {
      ...
      if( $request->gateway  == 'razorpay' ){
          ...
          if($request->after_discount==0)
          {
             ...
             $subscribe_onlineclasses = 0;
             if ( ! empty( $request->subscribe_onlineclasses ) ) {
              $subscribe_onlineclasses = 1;
             }
             $other_details['subscribe_onlineclasses'] = $subscribe_onlineclasses;
             if ( ! empty( $request->online_classes_price ) ) {
              $other_details['online_classes_price'] = $request->online_classes_price;
            }
    ...
    public function getModelName($type, $slug)
    {
        ...
        case 'package':
          ...
          break;
        case 'paidcontent':
          return \App\StudentPaidContent::where('slug', '=', $slug)->first();
          break;
    	}
    ...
    public function preserveBeforeSave($item, $package_type,$payment_method, $other_details,$coupon_zero=0)
    {
     ...
     if($other_details['paid_by_parent'])
      ...
      $payment->institute_id    = $user->institute_id;

      $subscribe_onlineclasses = 0;
      if ( ! empty( $other_details['subscribe_onlineclasses'] ) ) {
        $subscribe_onlineclasses = 1;
      }
      $payment->subscribe_onlineclasses = $subscribe_onlineclasses;

      if ( ! empty( $other_details['online_classes_price'] ) ) {
        $payment->online_classes_price = $other_details['online_classes_price'];
      }
    ...
    public function checkout($type, $slug)
    {
      ...
      $record = $this->getModelName($type, $slug);
        ...
        if(Payment::isItemPurchasedNotTrail($record->id, $type, $user->id))
        {
          ...
          $date = date('Y-m-d');
          $days = 0;
          $payments = \App\Payment::where('user_id', \Auth::id())->where('payment_status', 'success')->get();
          ...
          if ( $days > 5 ) {
            flash('Hey '.$user->name, 'you_already_purchased_this_item-checkout', 'overlay');
            return back();
          }
        }
        ...
        $active_class = 'lms';
        if($type == 'combo' || $type=='exams'|| $type=='exam' || $type == 'package' || $type == 'paidcontent' )
          $active_class = 'paid_content';
      	$data['active_class']       = $active_class;
        $data['pay_by']             = '';
        $data['title']              = $record->title;
        $data['item_type']          = $type;
        if ( $type == 'paidcontent' ) {
          $record->validity = $record->duration . ' ' . str_plural($record->duration_type, $record->duration);
        }
        $data['item']               = $record;
    ...
    public function updateOfflinePayment(Request $request)
    {
      ...
      $other_details['subscribe_onlineclasses'] = $payment_data->subscribe_onlineclasses;
      $other_details['online_classes_price'] = $payment_data->online_classes_price;
     ...
     public function getOnlinePaymentReportsDatatable($slug)
    {
      ...
      $institute_id   = adminInstituteId();
        ...
        if(checkRole(getUserGrade(3))){
         ...
         ->select(['payments.institute_id','users.image', 'users.name',  'item_name', 'plan_type', 'start_date', 'end_date', 'payment_gateway','payments.updated_at','payment_status','payments.cost', 'payments.after_discount', 'payments.paid_amount','payments.id' ])
         ...
         ->select(['users.image', 'users.name',  'item_name', 'plan_type', 'start_date', 'end_date', 'payment_gateway','payments.updated_at','payment_status','payments.cost', 'payments.after_discount', 'payments.paid_amount','payments.id' ])
         ->where('payment_gateway', '!=', 'offline')
         ->where('payments.institute_id',$institute_id)
         ->orderBy('updated_at', 'desc');
      ...
      if(!checkRole(getUserGrade(2)))
      {
       ...
       $institute_id   = adminInstituteId();
        ...
        if(checkRole(getUserGrade(3))){
           ...
           ->select(['payments.institute_id','users.image', 'users.name',  'item_name', 'plan_type', 'start_date', 'end_date', 'payment_gateway', 'payments.updated_at','payment_status','payments.id','payments.cost', 'payments.after_discount', 'payments.paid_amount'])
           ->where('payment_gateway', '=', 'offline')
           ->orderBy('updated_at', 'desc');
        }
        else{
    ...
    public function getRecordsCount($type, $status='')
    {
       ...
       if(checkRole('institute')){
          ...
          if($type=='online') {
            ...
            if($status=='')
              $count = Payment::where('payment_gateway', '!=', 'offline')->where('institute_id',$institute_id)->count();
    ...
    //  dd($request);
        ...
        if(count($input)  && !empty($input['razorpay_payment_id'])) {
            ...
            try {
                ...
                $payment_record->start_date = date('Y-m-d');
                if ( in_array($type, ['package', 'paidcontent']) ) {
                ...
                if( \Auth::user()->role_id == 7 && $type == 'package') {
                    ...
                    if ( $institute ) {
                      switch ($duration_type) {
                        case 'Day':
                            $institute->valid_until = now()->addDays($duration);
                            break;
                        case 'Week':
                            $institute->valid_until = now()->addWeeks($duration);
                            break;
                        case 'Month':
                            $institute->valid_until = now()->addMonths($duration);
                            break;
                        case 'Year':
                            $institute->valid_until = now()->addYears($duration);
                            break;
                        }
                        $institute->save();
                    }
