...
class DashboardController extends Controller
{
    ...
    public function index()
    {
      ...
      // dd('llll');
        ...
        $role = getRole();
        if($role=='admin' || $role=='owner' || $role == 'faculty' || $role == 'institute'){

              // Artisan::call('view:clear');

             $roles = App\Role::get()->pluck('id');
             if ( ! checkRole(getUserGrade(1)) ) {
                $roles = App\Role::whereIn('name', ['student', 'faculty'])->get()->pluck('id');
             }
             $dataset = [];
             $labels = [];
             $bgcolor = [];
             ...
             foreach($roles as $key => $value)
             {
                ...
                if ( checkRole(getUserGrade(1)) ) {
                  ...
                  $dataset[] = App\User::where('institute_id', adminInstituteId())->where('role_id', '=', $value)->get()->count();
                }
          ...
          else if($role == 'institute')
        {
          ...
          Artisan::call('view:clear');
             ...
             foreach($roles as $key => $value)
             {
                $color_number = rand(0,999);
                $labels[] = ucfirst(getRoleData($value));
                $dataset[] = App\User::where('role_id', '=', $value)
           ...
           // return view('admin.dashboard', $data);
            ...
            $view_name = getTheme().'::institutes.users.dashboard';
