...
class SubjectLogosController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['title']              = getPhrase('subject_logos');
        $data['layout']     = getLayout();

         $view_name = getTheme().'::exams.subjectlogos.list';
    ...
    public function getDatatable()
    {
        ...
        $records = SubjectLogos::select(['title', 'file_name', 'id']);
        ...
        if(checkRole(getUserGrade(2))) {
          ...
          if($records->institute_id  == $institute_id || checkRole(getUserGrade(2))){
            ...
            $temp.= '<li><a href="'.URL_SUBJECTLOGOS_EDIT.'/'.$records->id.'"><i class="fa fa-pencil"></i>'.getPhrase("edit").'</a></li>';
          }
    ...
    public function create()
    {
        ...
        $data['record']             = FALSE;
        $data['active_class']       = 'exams';
        $data['title']              = getPhrase('create_subject_logo');
        $data['layout']     = getLayout();

           $view_name = getTheme().'::exams.subjectlogos.add-edit';
        return view($view_name, $data);
    }
    ...
    {
        if(!canDo('subject_logos_edit'))
        {
          prepareBlockUserMessage();
          return back();
        }

        $record = SubjectLogos::getRecordWithSlug($slug);
        if($isValid = $this->isValidRecord($record))
            return redirect($isValid);

        $data['record']             = $record;
        $data['active_class']       = 'exams';
        $data['title']              = getPhrase('edit_subject_logo');
        $data['layout']     = getLayout();
       ...
       $this->validate($request, $rules);
        ...
        if ($request->hasFile($file_name))
        {
             ...
             $rules = array( $file_name => 'mimes:jpeg,jpg,png,gif|max:10000' );
              ...
              $record->file_name      = $this->processUpload($request, $record,$file_name);
              $record->save();
        }

        flash('success','record_updated_successfully', 'success');
        return redirect(URL_SUBJECTLOGOS);
    }
...
// catimage
    ...
    {
        if(!canDo('subject_logos_delete'))
        {
          prepareBlockUserMessage();
          return back();
        }

        $record = SubjectLogos::where('id', $id)->first();
            try{
            if(!env('DEMO_MODE')) {
                $this->setExamSettings();
                $examSettings = $this->getExamSettings();
                $path = $examSettings->categoryImagepath;
                $this->deleteFile($record->file_name, $path);
                $record->delete();
            }
            $response['status'] = 1;
