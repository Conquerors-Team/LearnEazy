...
class ReferenceBooksController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['title']              = getPhrase('reference_books');
        $data['layout']     = getLayout();

         $view_name = getTheme().'::exams.boards.ref-books.list';
    ...
    public function getDatatable()
    {
        ...
        if(checkRole(getUserGrade(3))) {
            ...
            $records = ReferenceBook::select([ 'title','author_name', 'id']);
        ...
        if(checkRole(getUserGrade(2))) {
          ...
          if($records->institute_id  == $institute_id || checkRole(getUserGrade(2))){
            ...
            $temp.= '<li><a href="'.URL_REF_BOOKS_EDIT.'/'.$records->id.'"><i class="fa fa-pencil"></i>'.getPhrase("edit").'</a></li>';
          }
    ...
    public function create()
    {
        ...
        $data['active_class']       = 'syllabus_contents';
        $institute_id   = adminInstituteId();
        $data['title']              = getPhrase('create_reference_books');
        $data['layout']     = getLayout();

           $view_name = getTheme().'::exams.boards.ref-books.add-edit';
        return view($view_name, $data);
    }
    ...
    public function edit($id)
    {
        ...
        $record = ReferenceBook::where('id',$id)->first();
        if($isValid = $this->isValidRecord($record))
            return redirect($isValid);

        $data['record']             = $record;
        $data['active_class']       = 'syllabus_contents';
        $data['title']              = getPhrase('edit_reference_books');
        $data['layout']     = getLayout();
    ...
    public function update(Request $request, $id)
    {
        ...
        $rules = [
         ...
         'fileinput'      => 'bail|mimes:png,jpg,jpeg,docx,pdf|max:2048'
          ];
         /**
       ...
       $this->validate($request, $rules);
        $record->title           = $name;
        $record->author_name  = $request->author_name;
        $record->save();
     ...
     public function processUpload(Request $request, $record, $file_name)
     {
        ...
        return ;
         ...
         if ($request->hasFile($file_name)) {
          ...
          $destinationPath = public_path(env('UPLOAD_PATH')) . '/uploads/ref-books';
