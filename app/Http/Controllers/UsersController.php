...
class UsersController extends Controller
{
  ...
  public function delete($slug)
  {
     ...
     */
    if (!env('DEMO_MODE')) {
      // Delete related records in users_subjects before deleting the user
      DB::table('users_subjects')->where('user_id', $record->id)->delete();
      DB::table('batch_faculty')->where('user_id', $record->id)->delete();
      $imageObject = new ImageSettings();
...
// dd($request->all());
  ...
  public function getParentsOnSearch(Request $request)
  {
    ...
    $role_id = getRoleData('parent');
    $records = App\User::where('name', 'LIKE', '%' . $term . '%')
