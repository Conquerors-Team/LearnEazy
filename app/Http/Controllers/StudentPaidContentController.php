...
class StudentPaidContentController extends Controller
{
  ...
  public function create()
  {
    ...
    $data['lms_contents'] = LmsContent::where('institute_id', $institute_id)->pluck('title', 'id')->toArray();
    $data['lms_series']   = LmsSeries::where('institute_id', $institute_id)->pluck('title', 'id')->toArray();
    $data['lms_notes']    = LmsNote::where('institute_id', $institute_id)->pluck('title', 'id')->toArray();
    $data['active_class']     = 'package';
  ...
  public function updateLmsSeries(Request $request, $slug)
  {
    ...
    $data['subjects'] = $list->pluck('subject_title', 'id')->toArray();


    $data['settings']           = FALSE;
    $previous_records = array();
    if ($record->total_items > 0) {
      $series = \App\StudentPaidContentData::select(['lmsseries.id', 'lmsseries.title', 'subjects.subject_title', 'chapters.chapter_name', 'topics.topic_name'])
    ...
    $data['exam_categories'] = App\QuizCategory::where('institute_id', $institute_id)->pluck('category','id')->toArray();
  ...
  public function updateLmsNotes(Request $request, $slug)
  {
    ...
    $previous_records = array();
    if ($record->total_items > 0) {
      $series = \App\StudentPaidContentData::select(['lms_notes.id', 'lms_notes.title', 'subjects.subject_title', 'chapters.chapter_name', 'topics.topic_name', 'lms_notes.content_type'])
  ...
  public function updateExams(Request $request, $slug, $type = '')
  {
    ...
    $previous_records = array();
    if ($record->total_items > 0) {
      $series = \App\StudentPaidContentData::select(['quizzes.id', 'quizzes.title', 'subjects.subject_title', 'chapters.chapter_name'])
      ...
      if ($type == 'previousyear') {
        $series = \App\StudentPaidContentData::select(['quizzes.id', 'quizzes.title'])
          ->join('quizzes', 'quizzes.id', '=', 'student_paid_contents_data.item_id')
          ->where('student_paid_contents_id', '=', $record->id);
