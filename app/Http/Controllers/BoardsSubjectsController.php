...
class BoardsSubjectsController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['title']              = getPhrase('board_subjects');
        $data['layout']     = getLayout();

         $view_name = getTheme().'::exams.boards.board-subjects.list';
    ...
    public function getDatatable()
    {
        if(!canDo('board_access'))
        {
          ...
          return back();
           ...
           $records = BoardSubject::select([ 'title','status', 'id']);
        ...
        if(checkRole(getUserGrade(2))) {
          ...
          if($records->institute_id  == $institute_id || checkRole(getUserGrade(2))){
            ...
            $temp.= '<li><a href="'.URL_BOARD_SUBJECTS_EDIT.'/'.$records->id.'"><i class="fa fa-pencil"></i>'.getPhrase("edit").'</a></li>';
          }
    ...
    public function create()
    {
        ...
        $data['record']             = FALSE;
        $data['active_class']       = 'syllabus_contents';
        $data['title']              = getPhrase('create_board_subject');
        $data['layout']     = getLayout();

           $view_name = getTheme().'::exams.boards.board-subjects.add-edit';
        return view($view_name, $data);
    }
    ...
    public function edit($id)
    {
        ...
        $record = BoardSubject::where('id',$id)->first();
        // dd($record);
        ...
        $data['record']             = $record;
        $data['active_class']       = 'syllabus_contents';
        $data['title']              = getPhrase('edit_board_subject');
        $data['layout']     = getLayout();
    ...
    public function update(Request $request, $id)
    {
       ...
       $this->validate($request, $rules);
        ...
        return redirect(URL_BOARDS_SUBJECTS);
    }
    ...
    public function store(Request $request)
    {
      // dd($request->all());
        ...
        $rules = [
          'title'          => 'bail|required|max:60',
          'subject_code' => 'bail|required|max:3'
            ];
        $this->validate($request, $rules);
        $record = new BoardSubject();
        // dd($record);
        $name                       =  $request->title;
        $record->title           = $name;
