...
class StudentLmsController extends Controller
{
    ...
    public function index()
    {
      if(checkRole(getUserGrade(2)))
      {
        ...
        if($cat_settings  == 'no'){
           ...
           flash('Ooops...!','no_LMS_categories_are_assigned_to_you_contact_your_admin','overlay');
           return back();
    ...
    public function subjects()
    {
      if(checkRole(getUserGrade(2)))
      {
        ...
        $data['active_class']       = 'lms';
        $data['title']              = 'LMS'.' '.getPhrase('subjects');
        $data['layout']              = getLayout();
        $data['subjects']         = [];
        $user = Auth::user();
        ...
        $view_name = getTheme().'::student.lms.subjects';
        return view($view_name, $data);
    ...
    public function viewSubjectItems($slug, $series_slug = '')
    { 
        $record = \App\Subject::getRecordWithSlug($slug);
        ...
        if ( ! empty( $series_slug ) ) {
          ...
          if($isValid = $this->isValidRecord($series)) {
            return redirect($isValid);
          }
          $data['series'] = $series;
        }
        ...
        /*
        $data['series']             = LmsSeries::join('batch_lmsseries', 'batch_lmsseries.lms_series_id', '=', 'lmsseries.id')
                                      ->where('subject_id','=',$record->id)
                                  ->whereIn('batch_id', $student_batches)
                                  ->paginate(getRecordsPerPage()); */
    ...
    public function viewSubjectItemsNotes($slug, $series_slug = '')
    {
        $record = \App\Subject::getRecordWithSlug($slug);
        ...
        $view_name = getTheme().'::student.notes.lmsnotes-series-list';
        return view($view_name, $data);
    }
