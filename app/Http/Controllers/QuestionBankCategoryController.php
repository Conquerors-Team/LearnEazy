...
class QuestionBankCategoryController extends Controller
{
    ...
    {
        if(!canDo('exams_category_access'))
        {
        ...
        $data['title']              = getPhrase('questionbank_categories');
        $data['layout']     = getLayout();

         $view_name = getTheme().'::exams.questionbankcategories.list';
    ...
    public function getDatatable()
    {
        ...
        if(checkRole(getUserGrade(3))) {
            ...
            $records = QuestionbankCategory::select(['institute_id', 'category', 'image', 'description', 'id','slug']);
            ...
            if(shareData()){
                 ...
                 $records = QuestionbankCategory::select(['category', 'image', 'description', 'id','slug'])
        ...
        if(checkRole(getUserGrade(2))) {
          ...
          if($records->institute_id  == $institute_id || checkRole(getUserGrade(2))){
            ...
            $temp.= '<li><a href="'.URL_QUIZ_CATEGORY_EDIT.'/'.$records->slug.'"><i class="fa fa-pencil"></i>'.getPhrase("edit").'</a></li>';
          }
    ...
    {
        if(!canDo('exams_category_create'))
        {
          prepareBlockUserMessage();
          return back();
        }
        ...
        $data['layout']     = getLayout();
           ...
           $view_name = getTheme().'::exams.questionbankcategories.add-edit';
        return view($view_name, $data);
    }
    ...
    {
         if(!canDo('exams_category_edit'))
        {
          prepareBlockUserMessage();
          return back();
        }

        $record = QuestionbankCategory::getRecordWithSlug($slug);
    ...
    public function update(Request $request, $slug)
    {
       ...
       $this->validate($request, $rules);
        ...
        return redirect(URL_QUESTIONBANK_CATEGORIES);
    }
...
// catimage
        ...
        $this->validate($request, $rules);
        $record = new QuestionbankCategory();
        $name                       =  $request->category;
        $record->category           = $name;
        $record->slug               = $record->makeSlug($name);
        $record->description        = $request->description;
    ...
    {
         if(!canDo('exams_category_delete'))
        {
          prepareBlockUserMessage();
          return back();
        }

        $record = QuestionbankCategory::where('slug', $slug)->first();
            try{
            if(!env('DEMO_MODE')) {
                $this->setExamSettings();
                $examSettings = $this->getExamSettings();
    ...
    {
        return URL_QUESTIONBANK_CATEGORIES;
    }
