...
class MessagesController extends Controller
{
    ...
    public function create()
    {
       ...
       if(getSetting('messaging_system_for','messaging_system')=='admin')
       {
        ...
        if(checkRole(getUserGrade(5))){ // If student

          $users   = User::where('id','!=',$user->id)
                          ->where('institute_id', $user->institute_id)
                          ->whereIn('role_id', [7,8])
                          ->get(); // Institute admin and faculty
        }

        if(checkRole(getUserGrade(8))){ // Institute

          $users   = User::where('id','!=',$user->id)
                          ->where('institute_id',$user->institute_id)
                          ->get(); // Students and Faculty of that institute.
        }
     ...
     * @return mixed
     */
    public function update($id, $request_id = '')
    {
        if(!getSetting('messaging', 'module'))
        {
            pageNotFound();
        ...
        if ( ! empty( $request_id ) ) {
            $renewal_request = \App\RenewalRequest::find( $request_id );
            if ( $renewal_request ) {
                $renewal_request->replied = 'yes';
                $renewal_request->save();
            }
        }

        if ( ! empty( $request_id ) ) {
            return redirect()->route('packages.renewal_requests');
        } else {
