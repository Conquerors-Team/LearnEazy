...
//dd( $data['chapters'] );
        ...
        ->editColumn('title',function($records)
        {
          ...
          if(count($records->subjects) > 0) {
            $subjects = '<p><span class="label label-info label-many">' . implode('</span><span class="label label-info label-many"> ',
                        $records->subjects->pluck('subject_title')->toArray()) . '</span></p>';
          }
          return $str . $subjects;
        })
    ...
    public function create( $exam_type = '' )
    {
      ...
      if( 'live_quizzes' == $exam_type ) {
        $data['categories']       = array_pluck(QuizCategory::where('id', QUIZTYPE_LIVEQUIZ)->get(), 'category', 'id');
      } else {
        $data['categories']       = array_pluck(QuizCategory::get(), 'category', 'id');
      }

      $data['instructions']       = array_pluck(App\Instruction::where('institute_id',$institute_id)->get(), 'title', 'id');
...
//
    ...
    public function getSubjectData(Request $request)
    {
      ...
      $questions = $subject_questions->get(['id', 'subject_id', 'topic_id', 'question_type', 'question',
   ...
   /**
    ...
    $record = Quiz::getRecordWithSlug($slug);
      ...
      $institute_id   = adminInstituteId();
      $data['subjects']       = array_pluck(Subject::where('institute_id',$institute_id)->get(), 'subject_title', 'id');
      if ( isFaculty() ) {
          $faculty_subjects = \App\User::find( \Auth::id() )->subjects()->pluck('subject_id')->toArray();
          $data['subjects']       = array_pluck(Subject::where('institute_id',$institute_id)->whereIn('id', $faculty_subjects)->get(), 'subject_title', 'id');
      }
      if(shareData('share_subjects')){
        $data['subjects']     = array_pluck(App\Subject::whereIn('institute_id', [$institute_id, OWNER_INSTITUTE_ID])->get(), 'subject_title', 'id');
        if ( isFaculty() ) {
          $faculty_subjects = \App\User::find( \Auth::id() )->subjects()->pluck('subject_id')->toArray();
          $data['subjects']     = array_pluck(App\Subject::whereIn('institute_id', [$institute_id, OWNER_INSTITUTE_ID])->whereIn('id', $faculty_subjects)->get(), 'subject_title', 'id');
        }
      }
