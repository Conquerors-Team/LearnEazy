...
app.controller('preparePacakges', function( $scope, $http, httpPreConfig) {
	...
	route = '{{route("pricing.student-packages")}}';
        data= {
                _method: 'post',
...
$scope.classChanged = function(selected_number) {
    ...
    $scope.getClassCrouses = function( class_id ) {
        if(class_id === undefined)
            return;
        route = '{{url("get-class-courses")}}/' + class_id;
...
function classChanged()
{
  ...
  route = '{{url("front-end/get-class-courses")}}/'+class_id;
...
function courseChanged()
{
    ...
    route = '{{url("pricing/student-packages")}}/'+course_id;
