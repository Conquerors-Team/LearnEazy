...
(function ($) {
    ...
    */
    ...
    var SBF = {
                    ...
                    SBF.ajax({
                        ...
                        if (response != false && Array.isArray(response)) {
                            if (!admin && this.isAgent(response[0]['user_type'])) {
                                $(area).find('.sb-info').html(sb_('You cannot sign in as an agent.')).sbActivate();
                            } else {
        ...
        event: function (name, parameters) {
            ...
            if (CHAT_SETTINGS['webhooks'] && ['SBLoginForm', 'SBUserDeleted', 'SBMessageSent', 'SBBotMessage', 'SBEmailSent', 'SBNewMessagesReceived', 'SBNewConversationReceived', 'SBNewConversationCreated', 'SBActiveConversationStatusUpdated', 'SBSlackMessageSent', 'SBMessageDeleted', 'SBRegistrationForm', 'SBRichMessageSubmit'].includes(name)) {
                SBF.ajax({
                    function: 'webhooks',
                    function_name: name,
    ...
    */
    ...
    var SBChat = {
        ...
        // Send a message
        sendMessage: function (user_id = -1, message = '', attachments = [], onSuccess = false) {
            let bot_unknow_notify = CHAT_SETTINGS['bot-unknow-notify'] && SBApps.dialogflowActive();
            let bot_status_code = bot_unknow_notify ? 1 : 2;
            ...
            if (SBChat.conversation == false) {
                SBChat.newConversation(admin ? 1 : bot_status_code, user_id, '', [], (admin && SB_ACTIVE_AGENT['department'] != '' ? SB_ACTIVE_AGENT['department'] : null), null, () => {
                    return this.sendMessage(user_id, message, attachments, onSuccess);
                });
                return false;
            }
            if (user_id == -1) {
            ...
            if (message != '' || attachments.length) {
                ...
                SBF.ajax({
                    ...
                    if ('status' in response && response['status'] == 'success') {
                        ...
                        if (!admin && is_user && activeUser().type == 'visitor') {
                            SBF.ajax({ function: 'update-user-to-lead', user_id: user_id }, () => {
                                ...
                                if (CHAT_SETTINGS['slack-active'] && send_slack) {
                                    this.slackMessage(user_id, activeUser().get('full_name'), activeUser().get('profile_image'), message, attachments);
                                }
                            });
                            ...
                            if (admin) {
                                ...
                                this.slackMessage(activeUser().id, (is_user ? activeUser().get('full_name') : CHAT_SETTINGS['bot-name']), (is_user ? activeUser().get('profile_image') : CHAT_SETTINGS['bot-image']), message, attachments);
                            }
                        }
                        ...
                        if (this.timetable && is_user && !admin && CHAT_SETTINGS['timetable'] && !CHAT_SETTINGS['office-hours']) {
                            setTimeout(() => {
                                if (is_conversation) {
                                    this.sendMessage(bot_id, '[timetable]');
                                    this.scrollBottom();
                ...
                if (is_user) {
                    message = message.replace('<script', '&lt;script').replace('</script', '&lt;/script').replace('javascript:', '').replace('onclick=', '').replace('onerror=', '');
                    $(chat).append((new SBMessage({ id: 'sending', profile_image: (admin ? SB_ACTIVE_AGENT['profile_image'] : activeUser().get('profile_image')), full_name: activeUser().get('full_name'), creation_time: '0000-00-00 00:00:00', message: message, user_type: (admin ? 'agent' : 'user') })).getCode().replace('<div class="sb-time"></div>', `<div class="sb-time">${sb_('Sending')}<i></i></div>`));
                ...
                setTimeout(() => {
                    SBF.ajax({
                        ...
                        if (!SBF.errorValidation(response)) {
                            ...
                            let is_unknow_notify = CHAT_SETTINGS['bot-unknow-notify'];
                            ...
                            // Slack
                            if (CHAT_SETTINGS['slack-active'] && 'messages' in response && Array.isArray(response['messages']) && (!is_unknow_notify || is_unknow || send_notification)) {
                                if (is_unknow || send_notification) {
                                    let last_user_message = this.conversation.getLastUserMessage();
                                    this.slackMessage(activeUser().id, activeUser().get('full_name'), activeUser().get('profile_image'), last_user_message.get('message'));
                                }
                                for (var i = 0; i < response['messages'].length; i++) {
                            ...
                            if (send_notification || (!admin && CHAT_SETTINGS['notify-agent-email'] && is_unknow_notify && is_unknow)) {
                                this.sendEmail(`${message}<br><br><span style="color:#a8a8a8;font-size: 12px;">${sb_('This message has been sent because the Dialogflow Bot does not know the answer to the user\'s question.')}</span>`);
                            }
        ...
        sendEmail: function (message, attachments) {
            if (!this.email_sent) {
                ...
                SBF.ajax({
                    ...
                    sender_name: admin ? SB_ACTIVE_AGENT['full_name'] : activeUser().get('full_name'),
                    sender_profile_image: admin ? SB_ACTIVE_AGENT['profile_image'] : activeUser().get('profile_image'),
                    message: message,
                    attachments: attachments
                }, () => {
        ...
        initChat: function () {
            getCurrentUser(true, () => {
                ...
                if (!admin && CHAT_SETTINGS['popup'] && !storage('popup') && (!responsive || !CHAT_SETTINGS['popup-mobile-hidden'])) {
                    this.popup();
                }
                ...
                if (CHAT_SETTINGS['header-name'] && active && user_type == 'user') {
                    $(chat_header).find('.sb-title').html(`${sb_('Hello')} ${activeUser().get('full_name')}!`);
                }
                if (active && CHAT_SETTINGS['welcome-trigger'] != 'open') {
                    this.welcome();
                }
                setInterval(() => {
                    this.updateConversations();
        ...
        openConversation: function (conversation_id) {
            activeUser().getFullConversation(conversation_id, (response) => {
                ...
                SBF.event('SBConversationOpen', { id: conversation_id, response: response });
            });
        },
                ...
                SBF.ajax({
                    ...
                    if (response.length) {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            ...
                            let conversation = new SBConversation([message], { 'id': conversation_id, 'conversation_status_code': status_code });
        ...
        headerAgent: function () {
            if (!admin && !this.dashboard && this.conversation != false && (this.agent_id == -1 || (this.conversation.getLastMessage() != false && SBF.isAgent(this.conversation.getLastMessage().get('user_type')) && this.conversation.getLastMessage().get('user_id') != this.agent_id))) {
            ...
            if (action == 'remove') {
                for (var i = 0; i < this.notifications.length; i++) {
                    if (this.notifications[i] == conversation_id) {
                        ...
                        if ([1, 2].includes(this.conversation.get('conversation_status_code'))) {
                            this.setConversationStatus(0);
                        }
                        break;
                    }
        ...
        showEmoji: function (button) {
            if ($(chat_emoji).sbTogglePopup(button)) {
                if (!admin) {
                    $(chat_emoji).css({ left: $(chat_editor).offset().left + 20, top: $(chat_editor).offset().top - window.scrollY - 304 });
                }
        ...
        privacy: function () {
            SBF.ajax({
                ...
                $(chat_scroll_area).append(`<div class="sb-privacy sb-init-form" data-decline="${response['decline'].replace(/"/g, '')}"><div class="sb-title">${response['title']}</div><div class="sb-text">${response['message']}</div>` + (response['link'] != '' ? `<a target="_blank" href="${response['link']}">${response['link-name']}</a>` : '') + `<div class="sb-buttons"><a class="sb-btn sb-approve">${response['btn-approve']}</a><a class="sb-btn sb-decline">${response['btn-decline']}</a></div></div>`);
                this.finalizeInit();
                SBF.event('SBPrivacy');
            });
            if (!this.dashboard) {
        ...
        followUp: function () {
            if (this.followUpCheck()) {
                ...
                }, CHAT_SETTINGS['office-hours'] ? 15000 : (SBApps.dialogflowActive() ? 8000 : 5000));
            }
        },
        ...
        registration: function (check = false, type = CHAT_SETTINGS['registration-required']) {
            ...
            $(chat_scroll_area).append(SBRichMessages.generate({}, type, 'sb-init-form'));
            if (!this.dashboard) {
                this.showDashboard();
            }
            this.dashboard = true;
    ...
    */
    ...
    var SBRichMessages = {
        ...
        generate: function (settings, name, css = '') {
            ...
            return `<div id="${id}" data-type="${name}" class="sb-rich-message sb-rich-${name} ${css}">` + ('title' in settings ? `<div class="sb-top">${sb_(settings['title'])}</div>` : '') + ('message' in settings ? `<div class="sb-text">${sb_(settings['message'])}</div>` : '') + `<div class="sb-content">${content}</div>` + ('success' in settings ? `<div data-success="${settings['success'].replace(/"/g, '')}" class="sb-info"></div>` : '') + `</div>`;
        },
            ...
            if (!admin && !$(element).sbLoading() && !this.is_busy) {
                ...
                switch (type) {
                    ...
                    case 'buttons':
                        ...
                        success = `${sb_(success == '' ? 'You selected:' : success)} *${settings}*`;
                        payload['rich-messages'][rich_message_id] = { type: type, result: settings };
                    ...
                    case 'inputs':
                        settings = SBForm.getAll(area);
                        success = `${sb_(success == '' ? 'You submitted the values below:' : success)} [list values="`;
                    ...
                    case 'registration':
                        ...
                        for (var key in user_settings) {
                            ...
                            if (value != '') {
                                ...
                                success += `${sb_(user_settings[key][1].replace(/:|,/g, ''))}:${value},`;
                            }
                        }
                        ...
                        error = area.find('#password').length ? 'Please insert your first name, last name, a valid email, and a password (8 character minimum).' : 'Please insert your first name, last name, and a valid email.';
                ...
                SBF.ajax(parameters, (response) => {
                    if (response != false && !SBF.errorValidation(response)) {
                        ...
                        } else if (!('type' in payload) || payload['type'] != 'close-message') {
                        ...
                        if (CHAT_SETTINGS['slack-active']) {
                            SBChat.slackMessage(activeUser().id, activeUser().get('full_name'), activeUser().get('profile_image'), success);
                        }
    ...
    $(document).ready(function () {
        ...
        if (main.length) {
            ...
            SBF.ajax({
                ...
                if (!admin && !CHAT_SETTINGS['chat-manual-init'] && (!CHAT_SETTINGS['disable-offline'] || CHAT_SETTINGS['agents-online']) && (!CHAT_SETTINGS['disable-office-hours'] || CHAT_SETTINGS['office-hours']) && (!CHAT_SETTINGS['chat-login-init'] || (!SBApps.isWP() && storage('login') != false) || (SBApps.isWP() && SBApps.isLoggedWP())) && (!SBApps.isWP() || CHAT_SETTINGS['wp-show-ids'].length == 0 || CHAT_SETTINGS['wp-show-ids'].includes(SB_WP_PAGE_ID))) {
                    SBChat.initChat();
                }
                SBF.event('SBReady');
            });
        ...
        $(main).on('click', '.sb-lightbox-overlay,.sb-lightbox-overlay > i', function () {
            $(main).find('.sb-lightbox-media').sbActivate(false);
            return false;
        });
