...
(function ($) {
            ...
            if (action == 'get') {
                ...
                $(items).each(function () {
                    ...
                    $(this).find('[data-id]').each(function () {
                        ...
                        if (empty && value != '' && $(this).attr('type') != 'hidden') {
                            empty = false;
                        }
    ...
    var SBUsers = {
        ...
        getRow: function (user) {
            if (user instanceof SBUser) {
                return `<tr data-user-id="${user.id}" data-user-type="${user.type}"><td><input type="checkbox" /></td><td class="sb-td-profile"><a class="sb-profile"><img src="${user.get('profile_image')}" /><span>${user.get('full_name')}</span></a></td><td class="sb-td-email">${user.get('email')}</td><td class="sb-td-ut">${user.type}</td><td>${SBF.beautifyTime(user.get('last_activity'))}</td><td>${user.get('creation_time')}</td></tr>`;
            } else {
        ...
        updateCurrentAgent: function (profile_image, first_name, last_name, email, department) {
            SBF.ajax({
                ...
                if (response != false) {
                    SBF.storage('login', response);
                }
            });
            SB_ACTIVE_AGENT['full_name'] = first_name + ' ' + last_name;
            SB_ACTIVE_AGENT['profile_image'] = profile_image;
            SBProfile.populateHeader($(header).find('.sb-account'), SB_ACTIVE_AGENT['full_name'], SB_ACTIVE_AGENT['profile_image']);
        }
    }
    ...
    */
    ...
    var SBProfile = {
        ...
        profileRow: function (key, value, name = key) {
            if (value == '') return '';
            let icons = { 'id': 'padlock', 'full_name': 'user', 'email': 'envelope', 'user_type': 'user', 'last_activity': 'reload', 'creation_time': 'calendar', 'token': 'shuffle', 'currency': 'currency', 'browser': 'smartphone', 'ip': 'internet', 'location': 'marker', 'country': 'marker', 'os': 'desktop', 'current_url': 'next', 'timezone': 'clock' };
            ...
            return `<li data-id="${key}">${icon}<span>${SBF.slugToString(name)}</span><label>${value}</label></li>`;
        },
        ...
        populateHeader: function (header, name = false, profile_image = false) {
            if (activeUser() != false) {
                if (name == false) name = activeUser().get('full_name');
                if (profile_image == false) profile_image = activeUser().get('profile_image');
            }
    ...
    */
    ...
    $(document).ready(function () {
        ...
        // Media lightbox
        $(admin).on('click', '.sb-lightbox-media > i', function () {
            $(admin).sbHideLightbox();
        }); 
        ...
        */
        ...
        $(conversations_admin_list_ul).on('click', 'li', function () {
            ...
            SBConversations.populate(conversation_id, $(this).attr('data-user-id'));
        ...
        $(admin).on('click', '.sb-user-conversations li', function () {
            ...
            SBConversations.populate(conversation_id, activeUser().id);
            ...
            if ($(select).find('.sb-active').attr('data-value') != status) {
                ...
                $(conversations_admin_list_ul).find('li').sbActivate(false);
                $(conversations_admin_list_ul).find(`[data-conversation-id="${conversation_id}"]`).sbActivate();
            }
            SBChat.email_sent = false;
        });
        ...
        */
            ...
            if (!$(this).sbLoading()) {
                ...
                SBF.ajax({
                    ...
                    if (new_user) {
                        user_id = response;
                        activeUser(new SBUser({ 'id': user_id }));
                    } else if (user_id == SB_ACTIVE_AGENT['id']) {
                        SBUsers.updateCurrentAgent(settings['profile_image'][0], settings['first_name'][0], settings['last_name'][0], settings['email'][0], ('department' in settings ? settings['department'][0] : null));
                    }
                    activeUser().update(() => {
                        users[user_id] = activeUser();
                        if (new_user) {
        ...
        $(profile_area).on('click', '.sb-start-conversation', function () {
            ...
            SBConversations.populate(-1, activeUser().id);
        });
        ...
        */
        ...
        $(settings_area).on('click', '#system-requirements a', function (e) {
            ...
            SBF.ajax({
                ...
                for (var key in response) {
                    code += `<div class="sb-input"><span>${SBF.slugToString(key)}</span><div${response[key] ? ' class="sb-green"' : ''}>${response[key] ? sb_('Success') : sb_('Error')}</div></div>`;
                }
                loading(false);
        ...
        $(settings_area).on('click', '#tab-translations', function () {
            SBF.ajax({
                ...
                for (var key in translations) {
                    ...
                    code += `<div data-code="${key}"><div data-area="front"><h2>${sb_('Front End')}<div><a class="sb-btn sb-icon sb-add-translation"><i class="sb-icon-plus"></i>${sb_('New translation')}</a></div></h2><div>`;
            ...
            $(this).sbUploadFiles(function (response) {
                ...
                if (response[0] == 'success') {
                    if ($(upload_target).closest('[data-type]').data('type') == 'upload-image') {
                        $(upload_target).attr('data-url', response[1]).css('background-image', `url(${response[1]})`);
                    }
                } else {
