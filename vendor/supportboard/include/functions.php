...
function sb_update_login($profile_image, $first_name, $last_name, $email, $department = '') {
    if (session_status() == PHP_SESSION_NONE) {
        session_start();
    }
...
function sb_add_user($settings = [], $settings_extra = [], $login_app = false) {
    ...
    $query = 'INSERT INTO sb_users(first_name, last_name, password, email, profile_image, user_type, creation_time, token, department, last_activity) VALUES ("' . sb_db_escape($settings['first_name']) . '", "' . sb_db_escape($settings['last_name']) . '", "' . sb_db_escape($settings['password']) . '", ' . ($settings['email'] == '' ? 'NULL' : '"' . $settings['email'] . '"') . ', "' . sb_db_escape($settings['profile_image']) . '", "' . $settings['user_type'] . '", "' . $now . '", "' . $token . '", ' . $settings['department'] . ', "' . $now . '")';
...
function sb_add_new_user_extra($user_id, $settings) {
    ...
    foreach ($settings as $key => $setting) {
        if (is_array($setting) && $setting[0] != '') {
            $query .= '("' . $user_id . '", "' . $key . '", "' . sb_db_escape($setting[1]) . '", "' . sb_db_escape($setting[0]) . '"),';
        }
    }
    if ($query != '') {
...
function sb_get_conversations($pagination = 0, $status_code = 0, $routing = false) {
    ...
    $result = sb_db_get('SELECT sb_messages.*, sb_users.user_type as message_user_type FROM sb_messages, sb_users, sb_conversations WHERE sb_users.id = sb_messages.user_id ' . $status_code . ' AND sb_conversations.id = sb_messages.conversation_id' . (sb_get_department() !== false ? ' AND sb_conversations.department = ' . sb_get_department() : '') . sb_routing_db($routing) . ' AND sb_messages.creation_time IN (SELECT max(creation_time) latest_creation_time FROM sb_messages GROUP BY conversation_id) GROUP BY conversation_id ORDER BY sb_conversations.status_code DESC, sb_messages.creation_time DESC LIMIT ' . ($pagination * 100 + $i) . ',100', false);
    if (isset($result) && is_array($result)) {
...
function sb_search_conversations($search, $routing) {
    ...
    $department = sb_get_department();
    $result = sb_db_get('SELECT sb_messages.*, sb_users.user_type as message_user_type FROM sb_messages, sb_users' . ($department !== false || $routing ? ', sb_conversations' : '') . ' WHERE sb_users.id = sb_messages.user_id' . ($department !== false ? ' AND sb_messages.conversation_id = sb_conversations.id AND sb_conversations.department = ' . $department : '') . sb_routing_db($routing) . ($routing ? ' AND sb_conversations.id = sb_messages.conversation_id' : '') .' AND (LOWER(sb_messages.message) LIKE "%' . $search . '%" OR LOWER(sb_messages.attachments) LIKE "%' . $search . '%" OR LOWER(sb_users.first_name) LIKE "%' . $search . '%" OR LOWER(sb_users.last_name) LIKE "%' . $search . '%" OR LOWER(sb_users.email) LIKE "%' . $search . '%") GROUP BY sb_messages.conversation_id ORDER BY sb_messages.creation_time DESC', false);
    if (isset($result) && is_array($result)) {
        return sb_get_conversations_users($result);
    } else {
...
function sb_new_conversation($user_id, $status_code = 0, $title = '', $department = -1, $agent_id = -1, $routing = false) {
    ...
    $conversation_id = sb_db_query('INSERT INTO sb_conversations(user_id, title, status_code, creation_time, department, agent_id) VALUES ("' . $user_id . '", "' . $title . '", "' . $status_code . '", "' . gmdate('Y-m-d H:i:s') . '", ' . (sb_isset_num($department) ? $department : 'NULL') . ', ' . (sb_isset_num($agent_id) ? $agent_id : 'NULL') . ')', true);
...
function sb_send_message($user_id = -1, $conversation_id, $message = '', $attachments = [], $conversation_status_code = -1, $payload = false, $queue = false) {
    ...
    if ($user_id != -1) {
        ...
        if ($security) {
            ...
            return ['status' => 'success', 'message-id' => $response, 'queue' => $queue];
        }
...
function sb_is_agent_typing($conversation_id) {
    return sb_db_get('SELECT id, first_name, last_name FROM sb_users WHERE typing = "' . $conversation_id . '"');
}
...
function sb_get_rich_messages_ids() {
    ...
    $ids = ['email' , 'registration' , 'login', 'timetable', 'articles', 'equation'];
...
function sb_get_rich_message($name) {
    if (in_array($name, ['email' , 'registration' , 'login', 'timetable', 'articles', 'equation'])) {
        $title = '';
        $message = '';
        $success = '';
        switch ($name) {
            ...
            case 'articles':
                ...
                break;
            case 'equation':
                $code = '<div class="sb-equation"><p>For what value of <em>k</em> will the equation&nbsp;&nbsp;</p>

<p><img src="data:image/png;base64," style="height:auto; width:auto" /><img src="data:image/png;base64," style="height:auto; width:auto" /> have equal roots&nbsp;</p>
</div>';
                break;
...
function sb_css() {
    ...
    if ($color_1 != '') {
        ...
        $css .= '.sb-chat .sb-dashboard>div>.sb-btn,.sb-editor .sb-bar-icons>div:hover:before,.sb-articles>div:hover>div,.sb-chat .sb-btn-text:hover, .sb-list .sb-btn-text:hover,.sb-editor .sb-submit,
                 ...
                 .sb-input.sb-input-image .image:hover:before,.sb-rich-message .sb-card .sb-card-btn,.sb-rich-message.sb-rich-slider .sb-arrow:hover { color: ' . $color_1 . '; }';
...
function sb_installation($details) {
    ...
    if ($connection_check === true) {
        ...
        $db_respones['conversations'] = $connection->query('CREATE TABLE IF NOT EXISTS sb_conversations (id int NOT NULL AUTO_INCREMENT, user_id INT NOT NULL, title VARCHAR(255), creation_time DATETIME NOT NULL, status_code TINYINT DEFAULT 0, department TINYINT,  PRIMARY KEY (id), FOREIGN KEY (user_id) REFERENCES sb_users(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci');
...
function sb_webhooks($function_name, $parameters) {
    if (isset($function_name) && $function_name != '' && isset($parameters)) {
        $names = ['SBLoginForm' => 'login', 'SBRegistrationForm' => 'registration', 'SBUserDeleted' => 'user-deleted', 'SBMessageSent' => 'message-sent', 'SBBotMessage' => 'bot-message', 'SBEmailSent' => 'email-sent', 'SBNewMessagesReceived' => 'new-message', 'SBNewConversationReceived' => 'new-conversation', 'SBNewConversationCreated' => 'new-conversation-created', 'SBActiveConversationStatusUpdated' => 'conversation-status-updated', 'SBSlackMessageSent' => 'slack-message-sent', 'SBMessageDeleted' => 'message-deleted',  'SBRichMessageSubmit' => 'rich-message'];
