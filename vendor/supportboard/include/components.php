    ...
    <div class="sb-main sb-chat sb-no-conversations<?php echo $css ?>" style="display: none; transition: none;">
        <div class="sb-body">
            <div class="sb-scroll-area">
                <div class="sb-header sb-header-main" <?php
    ...
    if ($header_type == 'agents' || $header_type == false) {
        ...
        echo '<div class="sb-profiles">' . $code . '</div>';
                ...
                <div class="sb-dashboard">
                    ...
                    <?php if (sb_get_setting('articles-active')) {
                              echo sb_get_rich_message('articles');
                          } ?>
                </div>
                <?php if (sb_get_setting('articles-active')) {
                          echo '<div class="sb-panel sb-panel-articles"></div>';
                      } ?>
            </div>
...
function sb_component_editor($admin = false) { ?>
    ...
    <div class="sb-editor">
        ...
        <div class="sb-textarea"><textarea class = "ckeditor" placeholder="<?php sb_e('Write a message...') ?>"></textarea> </div>
                        ...
                        <div class="sb-tab">
                            <div class="sb-nav">
                                ...
                                <ul>
                                    ...
                                    <?php if (defined('SB_WP')) echo '<li id="tab-wp">WordPress</li>' ?>
                                    <?php if (defined('SB_DIALOGFLOW')) echo '<li id="tab-dialogflow">Dialogflow</li>' ?>
                                    <?php if (defined('SB_SLACK')) echo '<li id="tab-slack">Slack</li>' ?>
                                    <li id="tab-apps">
                                        <?php sb_e('Apps') ?>
                                    </li>
                                    <li id="tab-articles">
            ...
            if (defined('SB_SLACK')) {
                ...
                echo '</div>';
                                ...
                                <div>
                                    <div class="sb-apps">
                                        <div data-app="slack">
                                            <?php if (defined('SB_SLACK')) {
                                                      echo '<i class="sb-icon-check"></i>';
                                                  } ?>
                                            <img src="<?php echo SB_URL ?>/media/apps/slack.svg" />
                                            <h2>Slack</h2>
                                            <p>
                                                <?php echo sb_('Communicate with your users right from Slack. Send and receive messages and attachments, use emojis, and much more.') ?>
                                            </p>
                                        </div>
                                        <div data-app="dialogflow">
                                            <?php if (defined('SB_DIALOGFLOW')) {
                                                      echo '<i class="sb-icon-check"></i>';
                                                  } ?>
                                            <img src="<?php echo SB_URL ?>/media/apps/dialogflow.svg" />
                                            <h2>Dialogflow</h2>
                                            <p>
                                                <?php echo sb_('Utilize an artificially intelligent chat bot to automatically answer your users\' questions and perform other complex tasks.') ?>
                                            </p>
                                        </div>
                                    </div>
                                </div>
...
function sb_departments($select = false) {
    $items = sb_get_setting('departments');
    $count = is_array($items) ? count($items) : 0;
    if ($count) {
        if ($select) {
            $code = '<div id="department" data-type="select" class="sb-input sb-input-select"><span>' . sb_('Department') . '</span><select><option value="">' . sb_('General') . '</option>';
            for ($i = 0; $i < $count; $i++) {
